...
public class Exc2 {
	...
	Exc2(){
				  ...
				  String userPwd="4444";
					  try
					  {
						  Class.forName(driverName);
						    ...
						    ResultSet rs=sm.executeQuery("select*from evens where zang='"+st3+"'and mi='"+st2+"'" );
						    if(rs.next()){
						     int i2=sm.executeUpdate("update evens set mi='"+st1+"' where zang='"+st3+"'");
						     p3.setText("修改成功");
						    }else{
						    	p3.setText("修改失败");
						    }
		...
		JLabel j3=new JLabel("输入新密码");
		JLabel jl=new JLabel("修改日历登录密码");
